...
def image_example():
    ...
    img_width, img_height = pdf.px_to_mm(150), pdf.px_to_mm(150)
    # draw image, center on page
    pdf.image_center(img=img, img_width=img_width, img_height=img_height, container_width=pdf.get_width_effective())
    # line breaks
    pdf.ln(50)
    # get cursor position
    x, y = pdf.get_x(), pdf.get_y()
    # draw a fixed table without content
    table_height = pdf.px_to_mm(200)
    # change color of table border
    pdf.set_draw_color(10, 10, 10)
    pdf.table_row(['', '', ''], option='fixed', fixed_height=table_height)
    # draw image no align
    pdf.image_center(img=img, x=x, y=y, img_width=img_width, img_height=img_height)
    # draw image center horizontally
    x = x + pdf.width_3()
    pdf.image_center(img=img, x=x, y=y, img_width=pdf.px_to_mm(150), img_height=pdf.px_to_mm(150),
                     container_width=pdf.width_3())
    # draw image center horizontally and vertically
    x = x + pdf.width_3()
    pdf.image_center(img=img, x=x, y=y, img_width=pdf.px_to_mm(150), img_height=pdf.px_to_mm(150),
                     container_width=pdf.width_3(), container_height=table_height)
